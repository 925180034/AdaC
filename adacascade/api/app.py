...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    raw_qdrant: AsyncQdrantClient | None = None
    local_llm_manager: local_llm_runtime.LocalLlmRuntimeManager | None = None
    ...
    try:
        ...
        # ── 3. Local vLLM idle monitor ────────────────────────────────────────
        local_llm_manager = local_llm_runtime.get_manager()
        app.state.local_llm_manager = local_llm_manager
        idle_monitor_task = asyncio.create_task(local_llm_manager.idle_monitor_loop())
        app.state.local_llm_idle_monitor = idle_monitor_task

        # ── 4. LangGraph checkpoint + compiled graph ──────────────────────────
        async with AsyncSqliteSaver.from_conn_string(settings.CKPT_PATH) as ckpt:
            ...
            # ── 5. Reconcile orphan INGESTED tasks ─────────────────────────────
        ...
        if local_llm_manager is not None:
            await local_llm_manager.stop_managed()
        if raw_qdrant is not None:
